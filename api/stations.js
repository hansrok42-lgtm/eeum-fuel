...
module.exports = async (req, res) => {
  // 같은 조건의 조회 결과는 Vercel CDN에서 10분간 재사용.
  // stale-while-revalidate로 캐시 만료 뒤에도 기존 결과를 잠시 보여주면서 갱신합니다.
  ...
  try {
    ...
    if (!targets.length) {
      return res.status(400).json({ ok: false, error: '잘못된 지역값입니다.' });
    }
